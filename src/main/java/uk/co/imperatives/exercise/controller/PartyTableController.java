...
public class PartyTableController {
    ...
    @Operation(summary = "Add a new table", description = "Creates a new party table. Requires the number of seats, and optionally the table number.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Table successfully created",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = PartyTable.class))),
    ...
    @PostMapping
    public ResponseEntity<PartyTable> addTable(
            @RequestBody(description = "Request payload for creating a new party table",
                    required = true,
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = AddTableRequest.class)))
